...
//JSON valid schema: http://json.schemastore.org/jsbeautifyrc
export default class Beautifier {
		...
		return new Promise((resolve, reject) => {
			...
			if(projectPath) {
				...
				fs.readFile(filename, "utf8", (err, data) => {
					...
					//Parsage de la configuration parsée en json:
					try {
	...
	js(projectPath, data) {
		...
		return this.checkConfig(projectPath).then((config) => {
			//On met à jour notre config en cas d'update de conf:
	...
	//Beautify sans configurations (utilisable en cas d'erreur: cf. provideFormatter()):
	jsDefault(data) {
		//On unset notre objet configuration pour ne pas prendre en
		//compte les anciennes configurations lors de prochains formattages:
		this.resetConfig();
